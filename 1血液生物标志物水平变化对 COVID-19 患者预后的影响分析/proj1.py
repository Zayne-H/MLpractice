...
from sklearn.metrics import mean_absolute_error, mean_squared_error, make_scorer,accuracy_score, f1_score
...
class RandomForestRegressionModel:
    def __init__(self, data_path, sheet_name=1):    #导入数据
        self.data_path = data_path
        self.sheet_name = sheet_name
        self.train_data = pd.read_excel(self.data_path, sheet_name=self.sheet_name)
        self.train_data.columns = self.train_data.columns.str.strip()
...
c_model = RandomForestModel(data_path='训练集.xlsx')
c_model.tune_hyperparameters()
c_model.cross_validate()


#covid_patients = c_model.features[c_model.labels == 1]
predicted_labels = c_model.rf_model.predict(c_model.features)

# 找出被分类为1的样本的索引
indices_of_class_1 = np.where(predicted_labels == 1)[0]

# 根据这些索引提取对应的特征
class_1_features = c_model.features.iloc[indices_of_class_1]
#git
